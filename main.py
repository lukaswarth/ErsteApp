...
class MyScreenManager(ScreenManager):
    ...
    def reset_game_access(self):
        conn = sqlite3.connect('kasino_game.db')
        c = conn.cursor()
        c.execute('UPDATE game_access SET is_paid = 0')
        conn.commit()
        conn.close()   
...
class Kasino(Screen):
    ...
    def button_ok(self):
        ...
        if int(self.ids.geld2.text) - 500 < 2000 * int(self.ids.schwer.text):
...
class GameOne(Screen):
    ...
    def button_s1(self):
        ...
        self.ids.geld4.text = str(int(self.ids.geld4.text) + 100000 * int(self.ids.schwer1.text))
...
class GameEight(Screen):
    ...
    def on_enter(self):
        ...
        if self.win8 == 1:
            btn = Button(
                ...
                color=[1, 0.84, 0, 1])
            btn.bind(on_press = self.statistik)
            self.ids.button_container.add_widget(btn)
    ...
    def statistik(self, instance):
        self.manager.current = "Statistiken"
        self.manager.transition.direction = "right"
...
class MainApp(App):
    def build(self):
        ...
        sm.add_widget(Clicker(name = "Clicker"))
        sm.add_widget(Statistiken(name = "Statistiken"))
    ...
    background_color = ListProperty([random.uniform(0, 1), random.uniform(0, 1), random.uniform(0, 1), 1])
    ...
    def transition_to(self, screen_name):
            ...
            self.manager.transition = FadeTransition()  # Set a valid transition
